...
[English](README.en.md) · [Mac App Store 中英文文案](docs/release/app-store-connect-config.template.md) · [图片与来源说明](store-assets/README.md)

AI Tip 面向需要反复理解论文、教材、技术规范与政策资料的读者。选中原文创建 Tip，沿着回答继续追问，再通过 Tip Tree 找回每一条思考分支。问题、解释和来源相伴，阅读过程不必变成散落的聊天记录。

![在文档原文旁阅读和提问；构图预览，待 Mac 实机复拍](store-assets/showcase/zh-CN/01-source.png)

## 围绕原文，一层层理解

- **原文锚定 Tip**：从句子、术语和论证开始，按需选择通俗、详细或专业解释；重新打开仍能回到对应位置。
- **从回答继续追问**：选中回复中的概念创建子 Tip。父聊天成为上下文，新问题在旁边展开；收回后返回上一层。
- **Tip Tree**：查看讨论分支、为节点命名、定位对话；每个 Tip 独立保存，可通过记忆开关参考其他 Tip 摘要。
- **复杂 PDF 阅读**：保留原始页面、图表和分页；原生文字可直接创建 Tip，扫描页可先进行本地中英文 OCR。可导出带 Tip 批注的 PDF 副本，不覆盖原件。
- **长文档上下文**：在本机检索与问题相关的片段，再提供给模型；不等于模型已经读完全部页面。
- **本地优先、自选模型**：支持 PDF、DOCX、Markdown 与文本。可用自己的模型 API，或下载/导入兼容 GGUF 本地模型。模型权重不预装，运行需足够内存和磁盘。

![父对话与子 Tip 并排展开；构图预览，待 Mac 实机复拍](store-assets/showcase/zh-CN/02-follow-up.png)

![用 Tip Tree 回到每一条思考分支；构图预览，待 Mac 实机复拍](store-assets/showcase/zh-CN/03-tree.png)

## 你决定数据去向

文档、编辑、Tip 和聊天默认保存在本机，登录不等于上传。云副本按文档主动上传，当前每用户云空间上限为 5 MB。联网搜索默认关闭，与在线模型调用是不同配置：即使搜索关闭，使用在线 API 时仍会把问题和相关文档上下文发送给所选服务商。外部服务可能收费。

没有模型也可以导入和阅读文档；生成 AI 回答需要可用 API 或已下载并加载的本地模型。OCR、AI 回答和计算辅助均有适用条件，不保证全部内容正确或资料始终最新。

![按需选择本地模型；构图预览，待 Mac 实机复拍](store-assets/showcase/zh-CN/04-models.png)

> 图中使用原创阅读示例与明确声明的演示回答，由真实客户端渲染并捕获，不是生成式 UI。当前在 Windows 上制作，属于 GitHub 展示与商店构图预览；**不是已签名 Mac 候选包的提交截图，也不是模型准确率证明**。Mac 上架前须复拍、核验沙箱和模型运行。

## 开发与实现说明

以下保留实现边界、构建命令和开发者说明。应用支持 Windows 桌面使用，并提供 Mac App Store 构建配置；不代表已经上架。

### 长文档与 Tip 等待体验（1.12.15）

原文件超过 10 MiB，或提取文字超过 40,000 字符时，自动启用本地中英 BM25 RAG：选区和有界邻近上下文始终保留，另检索最多 6 个相关片段进入模型实际输入。索引按用户、文档和内容摘要隔离；编辑或 OCR 更新后重建。片段及其页码/块 ID/版本摘要随回答保留，可在折叠工具记录中查看。这是关键词全文检索，不是向量语义检索；扫描 PDF 必须先有 OCR 文字，不把图片假装成可检索文本。

进入文档会后台准备本地上下文，并在启用 Python 工具时预热一个干净、一次性计算线程；不调用推理/Embedding API、不搜索互联网、不自动 OCR 全文。预热线程空闲 60 秒释放，执行过用户代码后立即销毁。流式提供方的普通回答与工具轮次均增量展示；工具参数完整且通过绑定验证后才执行。需要专业/引用审查的内容仍先缓冲。忽略流式参数而返回 JSON 的接口会明确提示，不重复请求同一回答。

停止生成会中止上游 HTTP、搜索及活动 Python Worker，未完成回答不作为完整消息保存。远端服务是否立即停止计算或计费仍取决于提供方。专项回归：先 `pnpm desktop:prepare`，再 `pnpm stream-rag:test`；受控提供方测试不代表所有真实模型的准确率或延迟评测。

## 运行桌面应用

开发和构建需要 Node.js 24+ 与 pnpm；安装后的桌面应用不需要用户另装 Node.js 或 Python。
...
Pyodide 核心、SymPy、Pandas 及其依赖随安装包分发，不依赖系统 Python。构建时根据固定版本锁文件准备并校验完整依赖闭包，安装后首次高级分析不再下载这些组件；缺失或损坏会明确报错，不会回退到 CDN。模型权重仍由用户选择下载，远程模型响应和联网检索需要相应网络条件。
